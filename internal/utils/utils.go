...
func BitsToUint8(bitsArr [6]byte) uint8 {
	var res uint8 = 0
	k := 0
	for i := 5; i >= 0; i-- {
		if bitsArr[i] == 1 {
			res += uint8(math.Pow(float64(2), float64(k)))
			//fmt.Println(uint8(math.Pow(float64(2), float64(k))))
		}
		k++
	}
...
func BitsToBytes(bitsArr [6]byte) [6]byte {
	len := len(bitsArr)
	var res [6]byte
	k := 0
	for i := len - 1; i >= 0; i-- {
		if bitsArr[i] == 1 {
			res[i] = byte(math.Pow(float64(2), float64(k)))
		}
		k++
	}
